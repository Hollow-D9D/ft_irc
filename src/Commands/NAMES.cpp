...
/*   Updated: 2022/10/05 13:18:53 by aavetyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string users_to_string(Channel channel)
{
    ...
    {
        if ((*it)->get_user_mode().has_mode(USER_MODE_INVISIBLE) != std::string::npos)
            continue;
...
void NAMES(Command &cmd)
{
    ...
    if (args.size() == 1)
    {
        ...
        if (channel != NULL)
        {
            ...
            if (users_string.length())
                sender.reply(353, channel_mode, channel->getName(), users_to_string(*channel));
            else
