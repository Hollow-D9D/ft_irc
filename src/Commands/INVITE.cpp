...
void INVITE(Command &cmd) {
  ...
  if (cmd.get_server()
          .get_channel(cmd.get_arguments()[0])
          .inChannel(cmd.get_arguments()[1]))
  ...
  if (cmd.get_server().is_channel(cmd.get_arguments()[1])) {
    ...
    channel.addInvitedUser(
        *(cmd.get_server().get_user(cmd.get_arguments()[0])));
  }
  cmd.get_sender().send_to(*(cmd.get_server().get_user(cmd.get_arguments()[0])),
                           "INVITE" + cmd.get_arguments()[0] + " " +
                               cmd.get_arguments()[1]);
  cmd.get_sender().reply(341, cmd.get_arguments()[0], cmd.get_arguments()[1]);
  if (cmd.get_server()
          .get_user(cmd.get_arguments()[0])
          ->get_user_mode()
          .has_mode(USER_MODE_AWAY))
